...
<body>
        ...
        <div class="layui-form-item">
          <div class="layui-inline">
            ...
            <div class="layui-input-block">
              <input type="text" name="nm" placeholder="请输入" autocomplete="off" class="layui-input">
            </div>
          </div>
          <div class="layui-inline">
  ...
  layui.use(['table', 'form'], function(){
	  ...
	  table.render({
	    elem: '#dg'
	    ,url: '${pageContext.request.contextPath}/cls/clsList?userId=${userId}' //数据接口
	    ,cols: [[ //表头
	      {type: 'checkbox', fixed: 'left'}
	      ,{title: '序号', width:60, type:'numbers',fixed: 'left'}
	      ,{field: 'cid', title: 'ID', width:80, sort: true, fixed: 'left',hide:true}
	      ,{field: 'nm', title: '名称', width:120,align:'center'}
	      ,{title: '操作', width: 250, align:'center', fixed: 'right', toolbar: '#line-btn'}
	    ]]
	      ...
	      batchdel: function(){
	        ...
	        //  layer.close(index);
	         ...
	         layer.confirm('确定删除吗？', function(index) {
	         	 ...
	         	 $.ajax({
			            type : "post",
			            url : "${pageContext.request.contextPath}/cls/del",
			            data:{'ids':ids},
			            async : true,
			            cache : false,
			            dataType : 'json',
		 ...
		 //添加
	      ,add: function(){
	        layer.open({
	          type: 2
	          ,title: '添加分类'
	          ,content: '${pageContext.request.contextPath}/cls/clsForm'
	          ,maxmin: true
	          ,area: ['700px', '650px']
	          ,btn: ['确定', '取消']
	          ,yes: function(index, layero){
	            ...
	            iframeWindow.layui.form.on('submit('+ submitID +')', function(data){
	              ...
	              $.ajax({
		            type : "post",
		            url : "${pageContext.request.contextPath}/cls/add",
		            data:data.field,
		            async : true,
		            cache : false,
		            dataType : 'json',
	    ...
	    table.on('tool(dg)', function(obj){
	      ...
	      if(obj.event === 'del'){
	        ...
	        layer.open({
	          type: 2
	          ,title: '编辑分类'
	          ,content: '${pageContext.request.contextPath}/cls/clsForm?cid='+data.cid
	          ,maxmin: true
	          ,area: ['500px', '450px']
	          ,btn: ['确定', '取消']
	          ,yes: function(index, layero){
	            ...
	            iframeWindow.layui.form.on('submit('+ submitID +')', function(data){
	              ...
	              //提交 Ajax 成功后，静态更新表格中的数据
	               $.ajax({
			            type : "post",
			            url : "${pageContext.request.contextPath}/cls/edit",
			            data:data.field,
			            async : true,
			            cache : false,
			            dataType : 'json',
